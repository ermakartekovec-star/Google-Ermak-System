...
ai_mode_active = {}  # Режим ИИ активен для пользователя
...
"""
    ...
    def handle_private_content_for_chats(message):
        ...
        # Проверяем, активирован ли режим ИИ для этого пользователя
        ...
        # Если это обычное текстовое сообщение и не ИИ режим, то показываем соответствующее меню
        if message.content_type == 'text':
            access_level = check_user_access(user_id)
            if access_level == 'admin':
...
def show_admin_menu(message):
    ...
    markup.add(btn_send, btn_chats, btn_email, btn_emails, btn_platon, btn_broadcast, btn_ai, btn_stop_ai, btn_settings)
...
if __name__ == "__main__":
    ...
    print("   4. ⏸️ Остановить ИИ (кнопка для выхода из режима)")
    print("   5. 📢 Рассылка всем пользователям бота (Telegram)")
    print("   6. 📧 Массовая EMAIL рассылка")
    print("   7. 📤 Отправка сообщений в Telegram чаты/каналы")
    print("   8. 👥 Управление email (админ)")
    print("   9. ⚙️ Настройки бота (админ)")
    print("   10. 💾 Все данные в Google Drive")
    print(f"{'=' * 60}")
