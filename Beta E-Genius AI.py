# E-Genius AI - ПОЛНЫЙ РАБОЧИЙ КОД СО ВСЕМИ ФУНКЦИЯМИ
import os
import sys
import time
import threading
...
PARENTAL_CONTROL_FILE = "parental_control.json"
PARENTAL_LOGS_FILE = "parental_logs.json"
...
parental_client = None
parental_monitoring = False
parental_thread = None
parental_notification_users = []  # Список родителей для уведомлений
...
def load_parental_config():
    """Загружает конфиг родительского контроля"""
    config = load_json_file(CONFIG_FILE, {})
    return config.get("PARENTAL_CONTROL", {})

def save_parental_config(data):
    """Сохраняет конфиг родительского контроля"""
    config = load_json_file(CONFIG_FILE, {})
    config["PARENTAL_CONTROL"] = data
    save_json_file(CONFIG_FILE, config)
    return True

def load_parental_data():
    """Загружает данные родительского контроля"""
    return load_json_file(PARENTAL_CONTROL_FILE, {
        "blocked_chats": [],
        "blocked_users": [],
        "activity_logs": [],
        "violations": [],
...
def save_parental_data(data):
    """Сохраняет данные родительского контроля"""
    return save_json_file(PARENTAL_CONTROL_FILE, data)

def load_parental_logs():
    """Загружает логи родительского контроля"""
    return load_json_file(PARENTAL_LOGS_FILE, {"logs": []})

def save_parental_logs(data):
    """Сохраняет логи родительского контроля"""
    return save_json_file(PARENTAL_LOGS_FILE, data)

def log_parental_event(event_type, user_id, details=""):
    """Логирует события родительского контроля"""
    logs = load_parental_logs()
    ...
    save_parental_logs(logs)

async def send_parental_notification(message):
    """Отправляет уведомление родителям"""
    try:
        config = load_parental_config()
        notify_parents = config.get("notify_parent", True)
        
        if notify_parents:
            # Получаем всех администраторов для уведомлений
        ...
        print(f"❌ Ошибка отправки родительского уведомления: {e}")

async def init_parental_client():
    """Инициализирует клиент родительского контроля"""
    global parental_client
    ...
    if not api_id or not api_hash:
        print("⚠️ Родительский контроль не настроен (нет api_id/api_hash)")
        return False
    
    try:
        if session_string:
            # Используем сохраненную сессию
            parental_client = TelegramClient(
        ...
        await parental_client.connect()
        
        if not await parental_client.is_user_authorized():
            print("❌ Родительский контроль: требуется авторизация")
            return False
        
        print("✅ Родительский контроль подключен")
        
        # Сохраняем сессию
        if not session_string:
            session_string = parental_client.session.save()
        ...
        print(f"❌ Ошибка инициализации родительского контроля: {e}")
        return False

async def check_parental_status():
    """Проверяет статус родительского контроля"""
    if not parental_client:
        return "❌ Не подключен"
    
    try:
        me = await parental_client.get_me()
        if me:
            return f"✅ Подключен как: {me.first_name or ''} {me.last_name or ''}"
        else:
            return "❌ Ошибка подключения"
    except:
        return "❌ Ошибка подключения"

async def get_child_dialogs():
    """Получает диалоги ребенка"""
    if not parental_client:
        return []
    
    try:
        dialogs = []
        async for dialog in parental_client.iter_dialogs(limit=50):
            dialogs.append({
                "id": dialog.id,
                "name": dialog.name,
...
async def get_child_bots():
    """Получает список всех ботов в аккаунте ребенка"""
    if not parental_client:
        return []
    
    try:
        bots = []
        async for dialog in parental_client.iter_dialogs(limit=100):
            if dialog.is_user:
                try:
                    user = await parental_client.get_entity(dialog.entity)
                    if hasattr(user, 'bot') and user.bot:
                        ...
                        # Проверяем, защищен ли бот
                        parental_data = load_parental_data()
                        protected_bots = parental_data.get("protected_bots", [])
                        
                        for protected_bot in protected_bots:
                            if protected_bot.get("username") == bot_info["username"]:
                                bot_info["is_protected"] = True
                                bot_info["protected_by"] = protected_bot.get("protected_by", "unknown")
                                break
...
def add_protected_bot(bot_username, protected_by="admin"):
    """Добавляет бота в защищенный список"""
    parental_data = load_parental_data()
    protected_bots = parental_data.get("protected_bots", [])
    
    # Проверяем, не добавлен ли уже бот
    for bot in protected_bots:
        if bot.get("username") == bot_username:
            return False
    
    protected_bots.append({
        "username": bot_username,
        "protected_by": protected_by,
        "protected_at": datetime.now().isoformat()
    })
    
    parental_data["protected_bots"] = protected_bots
    save_parental_data(parental_data)
    
    log_parental_event("BOT_PROTECTED", protected_by, f"Bot: @{bot_username}")
    return True

def remove_protected_bot(bot_username):
    """Удаляет бота из защищенного списка"""
    parental_data = load_parental_data()
    protected_bots = parental_data.get("protected_bots", [])
    
    original_count = len(protected_bots)
    protected_bots = [bot for bot in protected_bots if bot.get("username") != bot_username]
    
    if len(protected_bots) < original_count:
        parental_data["protected_bots"] = protected_bots
        save_parental_data(parental_data)
        
        log_parental_event("BOT_UNPROTECTED", "admin", f"Bot: @{bot_username}")
        return True
    
    return False

async def restore_protected_bot(bot_entity):
    """Восстанавливает защищенного бота"""
    try:
        # Отправляем сообщение боту, чтобы восстановить диалог
        await parental_client.send_message(bot_entity, "👋")
        
        # Получаем информацию о восстановлении
        async for dialog in parental_client.iter_dialogs(limit=10):
            if dialog.is_user and dialog.entity.id == bot_entity.id:
                await send_parental_notification(
                    f"✅ ВОССТАНОВЛЕН ЗАЩИЩЕННЫЙ БОТ\n\n"
                    f"• Бот: @{bot_entity.username or 'без username'}\n"
                    f"• Имя: {bot_entity.first_name or ''}\n"
        ...
        print(f"❌ Ошибка восстановления бота: {e}")
        return False

async def leave_blocked_chats():
    """Покидает заблокированные чаты"""
    config = load_parental_config()
    blocked_chats = config.get("blocked_chats", [])
    
    if not blocked_chats or not parental_client:
        return 0
    
    try:
        left_count = 0
        async for dialog in parental_client.iter_dialogs():
            if dialog.id in blocked_chats:
                try:
                    await parental_client.delete_dialog(dialog.id)
                    left_count += 1
                    print(f"✅ Покинул чат: {dialog.name}")
                except Exception as e:
                    print(f"❌ Не удалось покинуть чат {dialog.name}: {e}")
...
async def block_bad_users():
    """Блокирует плохих пользователей"""
    config = load_parental_config()
    blocked_users = config.get("blocked_users", [])
    
    if not blocked_users or not parental_client:
        return 0
    ...
    try:
        ...
        for user_id in blocked_users:
            try:
                await parental_client.block(user_id)
                blocked_count += 1
                print(f"✅ Заблокирован пользователь: {user_id}")
            except Exception as e:
                print(f"❌ Не удалось заблокировать {user_id}: {e}")
        
        return blocked_count
    except Exception as e:
        print(f"❌ Ошибка блокировки пользователей: {e}")
        return 0

async def start_parental_monitoring():
    """Запускает мониторинг в реальном времени"""
    global parental_monitoring
    
    if not parental_client:
        return False
    
    try:
        @parental_client.on(events.NewMessage)
        async def message_handler(event):
            await process_child_message(event)
        
        @parental_client.on(events.ChatAction)
        async def chat_action_handler(event):
            await process_chat_action(event)
        
        @parental_client.on(events.MessageDeleted)
        async def message_deleted_handler(event):
            await check_bot_deletion(event)
        
        parental_monitoring = True
        print("👁️ Родительский мониторинг запущен")
        return True
    except Exception as e:
        print(f"❌ Ошибка запуска мониторинга: {e}")
        return False

async def process_child_message(event):
    """Обрабатывает сообщения ребенка"""
    try:
        config = load_parental_config()
        
        # Проверяем отправителя
        sender_id = event.sender_id
        if sender_id in config.get("blocked_users", []):
            print(f"⛔ Сообщение от заблокированного пользователя: {sender_id}")
            await event.delete()
            return
        ...
        if event.text:
            ...
            for word in bad_words:
                if word in text_lower:
                    print(f"⛔ Сообщение с запрещенным словом: {word}")
                    ...
                    # Логируем нарушение
                    log_parental_event("VIOLATION", sender_id, f"Запрещенное слово: {word}")
                    
                    # Уведомляем родителя
                    await send_parental_notification(
                        f"🚫 НАРУШЕНИЕ В СООБЩЕНИИ\n\n"
                        f"• Чат: {chat_name}\n"
                        f"• Слово: {word}\n"
                        f"• Сообщение: {text_lower[:100]}...\n"
                        f"• Время: {datetime.now().strftime('%H:%M:%S')}"
                    )
...
async def process_chat_action(event):
    """Обрабатывает действия в чатах (вход ребенка в новые чаты)"""
    try:
        config = load_parental_config()
        ...
        child_user = await parental_client.get_me()
        
        # Проверяем тип события
        if event.user_joined:  # Кто-то вошел в чат
            if hasattr(event, 'user_id') and event.user_id == child_user.id:
                # Ребенок вошел в чат самостоятельно
                await handle_child_joined_chat(chat, "самостоятельно вошел")
        
        elif event.user_added:  # Кого-то добавили в чат
            if hasattr(event, 'user_id') and event.user_id == child_user.id:
                # Ребенка добавили в чат
                await handle_child_joined_chat(chat, "добавили")
        
    except Exception as e:
        print(f"❌ Ошибка обработки действия чата: {e}")

async def handle_child_joined_chat(chat, action):
    """Обрабатывает вход ребенка в новый чат"""
    try:
        chat_id = chat.id
        chat_name = chat.title if hasattr(chat, 'title') else "Неизвестный чат"
        chat_type = "канал" if hasattr(chat, 'megagroup') and chat.megagroup else "группа" if hasattr(chat, 'participants_count') else "личные сообщения"
        
        # Проверяем, не запрещен ли чат
        config = load_parental_config()
        if chat_id in config.get("blocked_chats", []):
            print(f"⛔ Ребенка {action} в запрещенный чат: {chat_name}")
            
            try:
                await parental_client.delete_dialog(chat_id)
                print(f"✅ Покинул запрещенный чат: {chat_name}")
                
                # Уведомляем родителя
                await send_parental_notification(
                    f"🚫 РЕБЕНКА {action.upper()} В ЗАПРЕЩЕННЫЙ ЧАТ\n\n"
                    f"• Чат: {chat_name}\n"
                    f"• Тип: {chat_type}\n"
                    f"• ID: {chat_id}\n"
                    f"• Действие: Автоматически покинул чат\n"
                    f"• Время: {datetime.now().strftime('%H:%M:%S')}"
                )
            except Exception as e:
                print(f"❌ Не удалось покинуть чат: {e}")
        else:
            # Уведомляем родителя о новом чате
            await send_parental_notification(
                f"📱 РЕБЕНКА {action.upper()} В НОВЫЙ ЧАТ\n\n"
                f"• Чат: {chat_name}\n"
                f"• Тип: {chat_type}\n"
                f"• ID: {chat_id}\n"
                f"• Время: {datetime.now().strftime('%H:%M:%S')}\n\n"
                f"<i>Для блокировки чата используйте меню '🚫 Управление блокировками'</i>"
            )
            
            # Логируем событие
            log_parental_event("NEW_CHAT", chat_id, 
                             f"Тип: {chat_type}, Название: {chat_name}, Действие: {action}")
            
    except Exception as e:
        print(f"❌ Ошибка обработки входа в чат: {e}")

async def check_bot_deletion(event):
    """Проверяет попытку удаления защищенного бота"""
    try:
        if event.deleted_ids:
            ...
            for deleted_id in event.deleted_ids:
                try:
                    entity = await parental_client.get_entity(deleted_id)
                    
                    if hasattr(entity, 'bot') and entity.bot:
                        bot_username = entity.username or ""
                        
                        # Проверяем, является ли бот защищенным
                        for protected_bot in protected_bots:
                            if protected_bot.get("username") == bot_username:
                                # Восстанавливаем диалог с ботом
                                await restore_protected_bot(entity)
                                
                                # Уведомляем родителя
                                await send_parental_notification(
                                    f"🚫 ПОПЫТКА УДАЛЕНИЯ ЗАЩИЩЕННОГО БОТА\n\n"
                                    f"• Бот: @{bot_username}\n"
                                    f"• Имя: {entity.first_name or ''}\n"
                                    f"• Действие: Автоматически восстановлен\n"
                                    f"• Время: {datetime.now().strftime('%H:%M:%S')}"
                                )
                                
                                log_parental_event("BOT_DELETION_ATTEMPT", deleted_id, 
                                                 f"Bot: @{bot_username}, Action: restored")
                                break
                except:
                    continue
    except Exception as e:
        print(f"❌ Ошибка проверки удаления бота: {e}")

async def generate_parental_report():
    """Генерирует отчет о активности ребенка"""
    if not parental_client:
        return "❌ Родительский контроль не подключен"
    ...
    try:
        ...
        dialogs = []
        dialog_count = 0
        async for dialog in parental_client.iter_dialogs(limit=30):
            dialog_count += 1
            if len(dialogs) < 10:  # Сохраняем только первые 10
                dialogs.append({
                    "name": dialog.name,
                    "is_channel": dialog.is_channel,
                    "is_group": dialog.is_group,
        ...
        # Получаем ботов
        bots = await get_child_bots()
        protected_bots_count = sum(1 for bot in bots if bot.get("is_protected", False))
        
        # Получаем логи
        logs = load_parental_logs()
        recent_logs = logs.get("logs", [])[-10:]  # Последние 10 записей
        
        # Формируем отчет
        report = f"""📊 ОТЧЕТ РОДИТЕЛЬСКОГО КОНТРОЛЯ
...
📱 Активность:
• Всего чатов/каналов: {dialog_count}
• Защищенных ботов: {protected_bots_count}/{len(bots)}
• Непрочитанных: {sum(d['unread'] for d in dialogs)}

🔒 Контроль:
• Заблокировано чатов: {len(load_parental_config().get('blocked_chats', []))}
• Заблокировано пользователей: {len(load_parental_config().get('blocked_users', []))}

📋 Последние чаты:"""
        
        for i, dialog in enumerate(dialogs, 1):
            type_icon = "📢" if dialog["is_channel"] else "👥" if dialog["is_group"] else "💬"
            report += f"\n{i}. {type_icon} {dialog['name']}"
            if dialog["unread"]:
                report += f" ({dialog['unread']} непрочитанных)"
        
        if recent_logs:
            report += "\n\n📜 Последние события:"
            for log in recent_logs[-5:]:  # Последние 5 событий
                time_str = datetime.fromisoformat(log["timestamp"]).strftime("%H:%M")
                report += f"\n• {time_str} - {log['event_type']}: {log['details']}"
        
        return report
        
    except Exception as e:
        print(f"❌ Ошибка генерации отчета: {e}")
        return f"❌ Ошибка генерации отчета: {str(e)[:100]}"
...
def load_config_from_drive():
    ...
    if not content:
        ...
        example_config = {
            ...
            "PARENTAL_CONTROL": {
                "api_id": 32552405,
                "api_hash": "a238676da52eb08907001eda5f9d6eee",
                "child_phone": "",
                "session_string": "",
                "blocked_chats": [],
                "blocked_users": [],
                "bad_words": ["мат", "нарко", "оружие", "порно", "мошенник"],
                "time_limits": {
                    "enabled": False,
                    "bedtime_start": "22:00",
                    "bedtime_end": "08:00",
...
# ========== ИНИЦИАЛИЗАЦИЯ РОДИТЕЛЬСКОГО КОНТРОЛЯ ==========
async def initialize_parental_control():
    """Инициализирует родительский контроль"""
    print("🔐 Инициализация родительского контроля...")
    
    config = load_parental_config()
    if not config.get("api_id") or not config.get("api_hash"):
        print("⚠️ Родительский контроль не настроен в config.json")
        return False
    
    success = await init_parental_client()
    if success:
        print("✅ Родительский контроль готов к работе")
        
        # Запускаем фоновые проверки
        run_async_in_thread(leave_blocked_chats())
        run_async_in_thread(block_bad_users())
        
        # Запускаем мониторинг
        run_async_in_thread(start_parental_monitoring())
        
        return True
    else:
        print("❌ Не удалось инициализировать родительский контроль")
        return False
...
def initialize_system():
    ...
    # Асинхронная инициализация родительского контроля
    def start_parental_init():
        asyncio.run(initialize_parental_control())
    
    global parental_thread
    parental_thread = threading.Thread(target=start_parental_init, daemon=True)
...
def setup_bot_handlers():
    ...
    @bot.message_handler(func=lambda message: message.text == "👨‍👩‍👧‍👦 Родительский контроль")
    def parental_menu_handler(message):
        """Меню родительского контроля"""
        user_id = message.from_user.id
        access_level = check_user_access(user_id)
        
        if access_level != 'admin':
            bot.send_message(message.chat.id, "<b>❌ Доступно только администратору.</b>")
            return
        
        markup = types.InlineKeyboardMarkup(row_width=2)
        buttons = [
            types.InlineKeyboardButton("🔐 Подключить аккаунт", callback_data="parental_connect"),
            types.InlineKeyboardButton("📋 Статус контроля", callback_data="parental_status"),
            types.InlineKeyboardButton("🚫 Управление блокировками", callback_data="parental_blocks"),
            types.InlineKeyboardButton("🤖 Управление ботами", callback_data="parental_bots"),
            types.InlineKeyboardButton("📊 Получить отчет", callback_data="parental_report"),
            types.InlineKeyboardButton("🔍 Проверить чаты", callback_data="parental_check_chats"),
            types.InlineKeyboardButton("🔄 Обновить блокировки", callback_data="parental_update_blocks"),
            types.InlineKeyboardButton("📱 Быстрая проверка", callback_data="parental_quick_check"),
            types.InlineKeyboardButton("🔙 Назад", callback_data="parental_back")
        ]
        markup.add(*buttons)
        ...
        bot.send_message(
            ...
            "Управление доступом ребенка к Telegram.\n\n"
            "<i>Для работы требуется подключить аккаунт ребенка</i>",
            reply_markup=markup
        )
...
"""
        ...
        elif call.data.startswith("parental_"):
            if access_level != 'admin':
                bot.answer_callback_query(call.id, "❌ Доступно только администратору")
                return
            
            if call.data == "parental_connect":
                ...
                if config.get("session_string"):
                    # Уже подключен
                    bot.edit_message_text(
                        "<b>✅ Аккаунт ребенка уже подключен</b>\n\n"
                        "Сессия сохранена в config.json\n\n"
                        "Хотите переподключиться?",
                        call.message.chat.id,
                        call.message.message_id
                    )
                else:
                    # Нужна авторизация
                    user_waiting_for_input[user_id] = 'parental_phone'
                    bot.edit_message_text(
                        "<b>📱 Подключение аккаунта ребенка</b>\n\n"
                        "Введите номер телефона ребенка (в формате +79001234567):\n\n"
                        "<i>При первом подключении потребуется код из SMS</i>",
                        call.message.chat.id,
                        call.message.message_id
                    )
            
            elif call.data == "parental_status":
                # Статус контроля
                async def check_status():
                    status = await check_parental_status()
                    bot.edit_message_text(
                        f"<b>📋 Статус родительского контроля</b>\n\n{status}",
                        call.message.chat.id,
                        call.message.message_id
                    )
                
                run_async_in_thread(check_status())
            
            elif call.data == "parental_blocks":
                # Управление блокировками
                markup = types.InlineKeyboardMarkup(row_width=2)
                buttons = [
                    types.InlineKeyboardButton("📵 Заблокировать чат", callback_data="parental_block_chat"),
                    types.InlineKeyboardButton("👤 Заблокировать пользователя", callback_data="parental_block_user"),
                    types.InlineKeyboardButton("🗑️ Удалить блокировку", callback_data="parental_remove_block"),
                    types.InlineKeyboardButton("📋 Список блокировок", callback_data="parental_list_blocks"),
                    types.InlineKeyboardButton("🔧 Настроить слова", callback_data="parental_set_words"),
                    types.InlineKeyboardButton("🔙 Назад", callback_data="parental_back")
                ]
                markup.add(*buttons)
                ...
                bot.edit_message_text(
                    ...
                    "Здесь вы можете управлять блокировками чатов и пользователей.",
                    call.message.chat.id,
                    call.message.message_id,
                    reply_markup=markup
                )
            
            elif call.data == "parental_bots":
                # Управление ботами
                async def show_bots_menu():
                    bots = await get_child_bots()
                    
                    if not bots:
                        bot.edit_message_text(
                            "<b>🤖 Управление ботами</b>\n\n"
                            "❌ В аккаунте ребенка не найдено ботов.",
                            call.message.chat.id,
                            call.message.message_id
                        )
                        return
                    ...
                    for bot_info in bots[:10]:  # Показываем первые 10 ботов
                        username = bot_info["username"]
                        name = bot_info["first_name"] or bot_info["title"]
                        is_protected = "🛡️" if bot_info.get("is_protected") else "🤖"
                        
                        button_text = f"{is_protected} {name}"
                        if username:
                            button_text += f" (@{username})"
                        
                        if bot_info.get("is_protected"):
                            markup.add(types.InlineKeyboardButton(
                                button_text, 
                                callback_data=f"parental_bot_unprotect_{username}"
                            ))
                        else:
                            markup.add(types.InlineKeyboardButton(
                                button_text, 
                                callback_data=f"parental_bot_protect_{username}"
                            ))
                    
                    markup.add(
                        types.InlineKeyboardButton("➕ Добавить защиту", callback_data="parental_bot_add"),
                        types.InlineKeyboardButton("📋 Защищенные боты", callback_data="parental_bot_protected"),
                        types.InlineKeyboardButton("🔙 Назад", callback_data="parental_back")
                    )
                    
                    bot.edit_message_text(
                        f"<b>🤖 Управление ботами ребенка</b>\n\n"
                        f"🛡️ - защищенный бот\n"
                        f"🤖 - обычный бот\n\n"
                        f"Найдено ботов: {len(bots)}",
                        call.message.chat.id,
                        call.message.message_id,
                        reply_markup=markup
                    )
                
                run_async_in_thread(show_bots_menu())
            
            elif call.data == "parental_report":
                # Получить отчет
                async def generate_report():
                    report = await generate_parental_report()
                    bot.edit_message_text(
                        report,
                        call.message.chat.id,
                        call.message.message_id
                ...
                run_async_in_thread(generate_report())
            
            elif call.data == "parental_check_chats":
                # Проверить чаты
                async def check_chats():
                    dialogs = await get_child_dialogs()
                    
                    if not dialogs:
                        bot.edit_message_text(
                            "<b>🔍 Проверка чатов</b>\n\n"
                            "❌ Не удалось получить чаты ребенка.",
                            call.message.chat.id,
                            call.message.message_id
                        )
                        return
                    
                    response = "<b>🔍 Чаты ребенка:</b>\n\n"
                    
                    for i, dialog in enumerate(dialogs[:15], 1):  # Показываем первые 15
                        type_icon = "📢" if dialog["is_channel"] else "👥" if dialog["is_group"] else "💬"
                        response += f"{i}. {type_icon} <b>{dialog['name']}</b>\n"
                        
                        if dialog["unread_count"]:
                            response += f"   📨 Непрочитанных: {dialog['unread_count']}\n"
                        
                        if dialog.get("last_message"):
                            msg_preview = dialog["last_message"][:50]
                            if len(dialog["last_message"]) > 50:
                                msg_preview += "..."
                            response += f"   💬 {msg_preview}\n"
                ...
                run_async_in_thread(check_chats())
            
            elif call.data == "parental_update_blocks":
                # Обновить блокировки
                async def update_blocks():
                    left_count = await leave_blocked_chats()
                    blocked_count = await block_bad_users()
                    
                    bot.edit_message_text(
                        f"<b>🔄 Обновление блокировок</b>\n\n"
                        f"✅ Покинуто чатов: {left_count}\n"
                        f"✅ Заблокировано пользователей: {blocked_count}",
                        call.message.chat.id,
                        call.message.message_id
                    )
                
                run_async_in_thread(update_blocks())
            
            elif call.data == "parental_quick_check":
                # Быстрая проверка
                async def quick_check():
                    if not parental_client:
                        bot.edit_message_text(
                            "<b>📱 Быстрая проверка</b>\n\n"
                            "❌ Родительский контроль не подключен.",
                            call.message.chat.id,
                            call.message.message_id
                        )
                        return
                    ...
                    try:
                        ...
                        dialogs_count = 0
                        async for _ in parental_client.iter_dialogs(limit=50):
                            dialogs_count += 1
                        
                        bots = await get_child_bots()
                        protected_count = sum(1 for bot in bots if bot.get("is_protected", False))
                        
                        response = f"""<b>📱 Быстрая проверка аккаунта</b>

👤 Ребенок: {me.first_name or ''} {me.last_name or ''}
📱 Username: @{me.username or 'нет'}

📊 Статистика:
• Чатов/диалогов: {dialogs_count}
• Ботов: {len(bots)}
• Защищенных ботов: {protected_count}
...
✅ Аккаунт активен"""
                        ...
                        bot.edit_message_text(
                            f"<b>❌ Ошибка проверки:</b> {str(e)[:100]}",
                            call.message.chat.id,
                            call.message.message_id
                        )
                
                run_async_in_thread(quick_check())
            
            elif call.data == "parental_back":
                bot.delete_message(call.message.chat.id, call.message.message_id)
                show_admin_menu(call.message)
        
        elif call.data.startswith("parental_bot_"):
            if call.data == "parental_bot_add":
                user_waiting_for_input[user_id] = 'parental_add_bot'
                bot.edit_message_text(
                    "<b>➕ Добавить защищенного бота</b>\n\n"
                    "Введите username бота (например: @example_bot):",
                    call.message.chat.id,
                    call.message.message_id
                )
            
            elif call.data == "parental_bot_protected":
                parental_data = load_parental_data()
                protected_bots = parental_data.get("protected_bots", [])
                
                if not protected_bots:
                    bot.edit_message_text(
                        "<b>🛡️ Защищенные боты</b>\n\n"
                        "❌ Нет защищенных ботов.",
                        call.message.chat.id,
                        call.message.message_id
                    )
                    return
                
                response = "<b>🛡️ Защищенные боты:</b>\n\n"
                
                for i, bot_info in enumerate(protected_bots, 1):
                    response += f"{i}. @{bot_info.get('username', 'unknown')}\n"
                    response += f"   Добавлен: {bot_info.get('protected_at', 'unknown')}\n\n"
                
                bot.edit_message_text(response, call.message.chat.id, call.message.message_id)
            
            elif call.data.startswith("parental_bot_protect_"):
                bot_username = call.data.replace("parental_bot_protect_", "")
                
                if add_protected_bot(bot_username, "admin"):
                    bot.answer_callback_query(call.id, f"✅ Бот @{bot_username} защищен")
                    ...
                    async def update_menu():
                        ...
                        bot.edit_message_text(
                            f"<b>🤖 Управление ботами ребенка</b>\n\n"
                            f"✅ Бот @{bot_username} теперь защищен!\n\n"
                            f"🛡️ - защищенный бот\n"
                            f"🤖 - обычный бот",
                            call.message.chat.id,
                            call.message.message_id,
                    ...
                    run_async_in_thread(update_menu())
                else:
                    bot.answer_callback_query(call.id, "❌ Бот уже защищен")
            
            elif call.data.startswith("parental_bot_unprotect_"):
                bot_username = call.data.replace("parental_bot_unprotect_", "")
                
                if remove_protected_bot(bot_username):
                    bot.answer_callback_query(call.id, f"✅ Защита с @{bot_username} снята")
                    ...
                    bot.answer_callback_query(call.id, "❌ Бот не найден в защищенных")
            ...
            bot.edit_message_text(
                f"<b>⚙️ Управление веб1-приложением Платона</b>\n\n"
    ...
    def handle_private_content_for_chats(message):
        ...
        if user_id in user_waiting_for_input:
            ...
            elif operation == 'parental_phone':
                process_parental_phone(message)
                del user_waiting_for_input[user_id]
                return
            
            elif operation == 'parental_code':
                process_parental_code(message)
                del user_waiting_for_input[user_id]
                return
            
            elif operation == 'parental_add_bot':
                process_parental_add_bot(message)
                del user_waiting_for_input[user_id]
                return
        ...
        menu_buttons = [
            ...
            "👨‍👩‍👧‍👦 Родительский контроль", "Платон🙌", "📢 Рассылка пользователям", 
            "🤖 ИИ-помощник", "⏸️ Остановить ИИ", "📧 Мои email", "⚙️ Настройки", 
            "🖥️ Управление ПК", "👍ERMAK APP", "🙌MAX_APP"
        ]
...
async def process_parental_phone(message):
    """Обработка номера телефона для родительского контроля"""
    ...
    try:
        # Сохраняем номер в конфиг
        config = load_parental_config()
        config["child_phone"] = phone
        save_parental_config(config)
        
        # Запрашиваем код
        global parental_client
        if parental_client and await parental_client.is_user_authorized():
            await parental_client.disconnect()
        ...
        # Отправляем код
        sent = await parental_client.send_code_request(phone)
        ...
        user_waiting_for_input[user_id] = 'parental_code'
        
        bot.send_message(message.chat.id,
                        "<b>✅ Код отправлен на телефон ребенка!</b>\n\n"
                        "Введите код из SMS:")
        
    except Exception as e:
        bot.send_message(message.chat.id, f"<b>❌ Ошибка: {str(e)[:100]}</b>")
        log_event("PARENTAL_PHONE_ERROR", user_id, f"Error: {str(e)}")

async def process_parental_code(message):
    """Обработка кода для родительского контроля"""
    code = message.text.strip()
    user_id = message.from_user.id
    
    if not code.isdigit():
        bot.send_message(message.chat.id, "<b>❌ Код должен содержать только цифры!</b>")
        user_waiting_for_input[user_id] = 'parental_code'
        return
    ...
    try:
        ...
        if not phone or not phone_code_hash:
            bot.send_message(message.chat.id, "<b>❌ Ошибка данных. Попробуйте снова.</b>")
            return
        
        # Авторизуемся
        await parental_client.sign_in(
            phone=phone,
            code=code,
        ...
        bot.send_message(message.chat.id,
                        "<b>✅ Аккаунт ребенка успешно подключен!</b>\n\n"
                        "Сессия сохранена. Теперь вы можете использовать все функции родительского контроля.")
        
        # Запускаем мониторинг
        await start_parental_monitoring()
        ...
        error_msg = str(e)
        if "Two-steps verification" in error_msg:
            bot.send_message(message.chat.id,
                            "<b>🔐 Требуется двухфакторная аутентификация</b>\n\n"
                            "Введите пароль от аккаунта ребенка:")
            user_waiting_for_input[user_id] = 'parental_password'
        else:
            bot.send_message(message.chat.id, f"<b>❌ Ошибка: {error_msg[:100]}</b>")
            log_event("PARENTAL_CODE_ERROR", user_id, f"Error: {error_msg}")

def process_parental_add_bot(message):
    """Обработка добавления защищенного бота"""
    ...
    if add_protected_bot(bot_username, "admin"):
        bot.send_message(message.chat.id,
                        f"<b>✅ Бот {bot_username} добавлен в защищенные!</b>\n\n"
                        "Теперь ребенок не сможет его удалить.")
    else:
        bot.send_message(message.chat.id,
                        f"<b>⚠️ Бот {bot_username} уже защищен.</b>")
...
def show_admin_menu(message):
    ...
    btn_parental = types.KeyboardButton("👨‍👩‍👧‍👦 Родительский контроль")
    ...
    if platon_app_enabled:
        ...
        markup.add(btn_pc, btn_send, btn_chats, btn_email, btn_emails, 
                   btn_parental, btn_platon, btn_broadcast, btn_ai, btn_stop_ai, 
                   btn_settings, btn_ermak, btn_max_app)
    else:
        markup.add(btn_pc, btn_send, btn_chats, btn_email, btn_emails, 
                   btn_parental, btn_platon, btn_broadcast, btn_ai, btn_stop_ai, 
                   btn_settings, btn_ermak)
...
✅ 👨‍👩‍👧‍👦 Родительский контроль (мониторинг аккаунта ребенка)
...
if __name__ == "__main__":
    ...
    print("   11. 👨‍👩‍👧‍👦 Родительский контроль (мониторинг ребенка)")
    print("   12. ⚙️ Настройки бота (админ)")
    print("   13. 🖥️ Управление ПК (админ)")
    print("   14. ⏰ Отложенная отправка сообщений Платону")
    print("   15. ⏰ Отложенная EMAIL рассылка")
    print("   16. 🔒 Защищенный доступ к веб-приложению MAX")
    print("   17. 🤖 Управление ботами ребенка (защита от удаления)")
    print("   18. 📱 Мгновенные уведомления о новых чатах ребенка")
    print("   19. 💾 Все данные в Google Drive")
    print(f"{'=' * 60}")
