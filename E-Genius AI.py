...
ai_mode_active = {}
user_selected_emails = {}  # Для выбора email адресов
user_temp_data = {}  # Для временных данных
...
def add_delayed_task(task_type, target_id, message, delay_seconds, user_id, additional_data=None):
    ...
    task = {
        "id": str(uuid.uuid4()),
        "type": task_type,  # "platon_message" или "email_broadcast"
        "target_id": target_id,  # user_id для platon_message, список emails для email_broadcast
        "message": message,
        ...
        if task["status"] == "scheduled":
            try:
                ...
                if delay_seconds > 0:
                    ...
                    # Задача просрочена, выполняем немедленно
                    task["status"] = "overdue"
                    save_delayed_tasks(tasks)
            except Exception as e:
...
def toggle_platon_app(enabled):
    ...
    # Уведомляем всех пользователей Платона
    if not enabled:
        platon_ids = get_platon_users()
        for user_id in platon_ids:
            try:
                bot.send_message(user_id, "⛔ Веб-приложение 🙌MAX_APP временно отключено администратором.")
                # Пересылаем меню без кнопки веб-приложения
...
def setup_bot_handlers():
    ...
    def platon_app_handler(message):
        ...
        app_button = types.InlineKeyboardButton(
            "🌐 Открыть MAX APP", 
            url="https://t.me/Ermak_MAX_bot/platon_app"
        )
        markup.add(app_button)
        
        bot.send_message(
            message.chat.id,
            "<b>🚀 Откройте MAX APP прямо сейчас!</b>\n\n"
            "Нажмите на кнопку ниже, чтобы мгновенно открыть веб-приложение:",
            reply_markup=markup
        )
...
"""
            ...
            # Начинаем процесс выбора email для рассылки
            ...
            # Сохраняем список email для пользователя
            ...
            # Обработка выбора/снятия выбора email
            ...
            # Выбор времени задержки для email рассылки
            ...
            # Начало выборочной email рассылки
            emails_list = get_emails_with_users()
            ...
            # Выбор времени для массовой рассылки
            ...
            # Включение/выключение веб-приложения Платона
            ...
            # Если выключили, уведомляем всех пользователей Платона
            if not new_status:
        ...
        # Обработка функций для Платона
            ...
            bot.edit_message_text(
                ...
                f"• Доступ к приложению будет заблокирован",
                call.message.chat.id,
                call.message.message_id,
                reply_markup=markup
            )
    ...
    def show_email_selection_page(message, user_id, page_num):
        ...
        # Добавляем email адреса с чекбоксами
...
def process_delayed_platon(message):
    ...
    if delay_seconds > 0:
        ...
        task_id = add_delayed_task(
            ...
            target_id=platon_ids[0],  # Отправляем первому пользователю Платону
...
def show_admin_menu(message):
    ...
    # Добавляем кнопку MAX APP только если она включена
...
def show_platon_menu(message_or_user_id):
    ...
    if platon_app_enabled:
        btn_max_app = types.KeyboardButton("🙌MAX_APP")
        markup.add(btn_ai, btn_stop_ai, btn_email, btn_ermak, btn_max_app)
    else:
...
if __name__ == "__main__":
    ...
    print("   9. 📤 Отправка сообщений в Telegram чаты/каналы (из chats.db)")
    print("   10. 👥 Управление email (админ)")
    print("   11. ⚙️ Настройки бота (админ)")
    print("   12. 🖥️ Управление ПК (админ) - выключение, скриншоты")
    print("   13. ⏰ Отложенная отправка сообщений Платону")
    print("   14. ⏰ Отложенная EMAIL рассылка")
    print("   15. 💾 Все данные в Google Drive")
    print(f"{'=' * 60}")
